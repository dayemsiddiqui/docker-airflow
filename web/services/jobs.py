...
def fetch_latest_jobs(page=1, per_page=10):
    jobs = db.jobs.find().sort('date', pymongo.DESCENDING).skip((page - 1) * per_page).limit(per_page)
    jobs = list(jobs)
...
def search_jobs(search, page=1, per_page=10):
    jobs = db.jobs.find({"$text": {"$search": search}}).sort('date', pymongo.DESCENDING).skip((page - 1) * per_page).limit(per_page)
    jobs = list(jobs)
    jobs = json.dumps({'result': jobs}, indent=4, default=json_util.default)
